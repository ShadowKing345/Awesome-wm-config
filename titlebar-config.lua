...
local function title_buttons(c)
  return awful.util.table.join(awful.button({}, 1, function()
    client.focus = c;
    c:raise()
...
function titlebar:init()
  ...
  style.mark_mini = redutil.table.merge(redutil.table.check(beautiful, "titlebar.mark") or {}, {size = 30, gap = 10, angle = 0})
  style.icon = redutil.table.merge(redutil.table.check(beautiful, "tablebar.icon") or {}, {gap = 10})
  ...
  client.connect_signal("request::titlebars", function(c)
    ...
    -- build mini titebar model
    local base = wibox.widget({
      nil,
      {right = style.mark_mini.gap, redtitle.mark.focus(c, style.mark_mini), layout = wibox.container.margin},
      {
