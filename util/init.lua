...
function util.string.split(s, sep)
  ...
  for match in (s .. sep):gmatch("(.-)" .. sep) do table.insert(result, match) end
...
function util.table.subset(t, i, j)
  ...
  for x = i, j, 1 do table.insert(result, t[x]) end
...
function util.tblToJson(tbl, indent, isObj)
  ...
  if not isObj then isObj = false end
...
function util.table.indexOf(tbl, item)
  ...
  for index, value in ipairs(tbl) do if item == value then i = index end end
