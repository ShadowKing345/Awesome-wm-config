...
function bindingConfig:new(env)
  ...
  self.keys = {
    global = gTJoin( -- Tags
    aKey {modifier = {env.modKey}, key = "Left", callback = awful.tag.viewprev, description = {description = "View previous", group = "Tag"}},
    aKey {modifier = {env.modKey}, key = "Right", callback = awful.tag.viewnext, description = {description = "View next", group = "Tag"}},
    -- Launcher
    aKey {
      modifier = {env.modKey},
      key = "Return",
    ...
    }, aKey {modifier = {env.modKey, "Shift"}, key = "q", callback = awesome.quit, description = {description = "Quit Awesome", group = "Awesome"}},
    aKey {modifier = {env.modKey}, key = "s", callback = hotkeysPopup.show_help, description = {description = "Show help", group = "Awesome"}}),
    client = gTJoin(aKey {
      modifier = {"Mod1"},
      key = "F4",
      callback = function(c)
  ...
  for i = 1, 9 do
    self.keys.global = gTJoin(self.keys.global, -- View tag only.
    aKey {
      modifier = {env.modKey},
      key = ("#" .. i + 9),
      callback = function()
  ...
  self.mouse = {
    global = gTJoin(aButton {modifier = {}, button = 4, callback = awful.tag.viewnext},
                    aButton {modifier = {}, button = 5, callback = awful.tag.viewprev}),
    client = gTJoin(aButton {
      modifier = {},
      button = 1,
...
function bindingConfig.mt:__call(...)
  return bindingConfig:new(...)
end
